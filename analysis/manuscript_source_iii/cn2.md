...
#N **队列检索策略。** 正文现在写的是"从院内手术记录数据库检索并逐份复核手术记录"，但**具体用了什么诊断编码或检索词，我无法从数据里反推**，不能替你编。STROBE 第 6 条要求写明。请补一句，例如"以术中诊断含'肠旋转不良'或术式含'Ladd 手术'检索，共复核 XXX 份手术记录"。这是全文唯一一处仍缺的、只有你能提供的信息。
